...
resume_file = '/home/ans/PycharmProjects/Depth_in_The_Wild/resduc_c1.pth.tar'
test_norm = True
# test_norm = False
if resume_from_file:
...
with torch.no_grad():
    for i_batch, sample_batched in enumerate(test_loader):
        ...
        rdxs = (input_gt_depth_image > 8) & (input_gt_depth_image < 10)

        input_gt_depth_image = input_gt_depth_image[rdxs]
        pred_depth_image = pred_depth_image[rdxs]

        if len(input_gt_depth_image) < 1:
            print('input have no value in this range')
            continue
        ...
        # plt.imsave('./results/gt/Test_gt_depth_{:05d}.png'.format(idx), input_gt_depth_image)
        #     plt.imsave('./results/pred/Test_pred_depth_{:05d}.png'.format(idx), pred_depth_image, cmap='gray')
        # fig = plt.figure()
        ...
        # pred_depth_image = pred_depth_image[rdxs]
        # input_gt_depth_image = input_gt_depth_image[rdxs]
        # pred_d_gt = pred_d_gt[rdxs]
        # gt_d_pred = gt_d_pred[rdxs]
        ...
        if np.isnan(pred_depth_image).sum() > 0:
            print('pred nan')
            continue
        # if np.isnan(input_gt_depth_image).sum() > 0:
        #     continue
